...
// In production, the Express server serves the built app on the same origin.
// ---------------------------------------------------------------------------
...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  // In dev the backend (NODE_ENV=development) accepts x-bypass-auth.
  // For a production build, set VITE_ADMIN_API_KEY at build time.
  if (ADMIN_API_KEY) {
