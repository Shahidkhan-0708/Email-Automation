...
import { runResearchSynchronously } from '../services/research.service.js';
...
personalizationRouter.post('/personalization/generate/:profileId', requireAuth, async (req, res) => {
  try {
...
personalizationRouter.get('/personalization/:personalizationId/evidence', requireAuth, async (req, res) => {
  try {
...
personalizationRouter.put('/personalization/regenerate', requireAuth, async (req, res) => {
  try {
...
personalizationRouter.get('/review/queue', requireAuth, async (req, res) => {
  try {
...
personalizationRouter.post('/review/:personalizationId', requireAuth, async (req, res) => {
  try {
    ...
    const isInvalid = err.message.startsWith('Invalid decision') || err.message.startsWith('Invalid decidedBy');
    res.status(isInvalid ? 400 : 500).json({ error: err.message });
  }
});

// POST /api/bulk/approve-and-send - approve pending personalizations and trigger the send
personalizationRouter.post('/bulk/approve-and-send', requireAuth, async (req, res) => {
  try {
...
personalizationRouter.get('/bulk/progress', requireAuth, async (req, res) => {
  try {
...
// POST /api/enrichment/:profileId/run - synchronous research run for a profile
// (kept for compatibility; the Research UI uses the async /api/research/:id/run)
personalizationRouter.post('/enrichment/:profileId/run', requireAuth, async (req, res) => {
  try {
    await getProfile(req.params.profileId);
    const result = await runResearchSynchronously(req.params.profileId);
    res.json({ success: true, results: result.evidence || [] });
  } catch (err) {
